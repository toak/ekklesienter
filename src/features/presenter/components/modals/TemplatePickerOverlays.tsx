        ...
        <div className="absolute inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-md">
          <div className="bg-stone-800 border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              <input autoFocus type="text" value={props.newName} onChange={(e) => props.setNewName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') props.confirmSaveTemplate(); if (e.key === 'Escape') props.setIsNamingTemplate(false); }} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={`${t('common:name')}...`} />
              {props.isDev && <input type="text" value={props.newNameRu} onChange={(e) => props.setNewNameRu(e.target.value)} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={`${t('common:name')} (RU)...`} />}
              <div>
                ...
                <DropdownSelector
                  ...
                  onChange={(val) => props.setTargetBlockId(val || undefined)}
                  options={[
                    { value: '', label: t('no_block') },
                    ...props.allBlocks.map(b => ({
                      value: b.id,
                      label: `${b.icon} ${props.isRu ? b.nameRu : b.name}`
                    }))
                  ]}
                  className="py-2.5"
                />
        ...
        <div className="absolute inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-md">
          <div className="bg-stone-800 border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1 block">{t('template_name')}</label>
                <div className="grid grid-cols-2 gap-2">
                  <input autoFocus type="text" value={props.editName} onChange={(e) => props.setEditName(e.target.value)} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={t('common:name')} />
                  <input type="text" value={props.editNameRu} onChange={(e) => props.setEditNameRu(e.target.value)} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={`${t('common:name')} (RU)`} />
                </div>
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1 block">{t('template_id')}</label>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <label className="text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1 block">{t('category')}</label>
                  <DropdownSelector
                    value={props.editCategoryId || ''}
                    onChange={(val) => props.setEditCategoryId(val || undefined)}
                    options={[
                      { value: '', label: t('no_block') },
                      ...props.allBlocks.map(b => ({
                        value: b.id,
                ...
                <div className="shrink-0">
                  <label className="text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1 block">{t('background')}</label>
      ...
      {props.editingLayout && (
        <div className="absolute inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-md">
          <div className="bg-stone-800 border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              <input autoFocus type="text" value={props.editName} onChange={(e) => props.setEditName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') props.handleUpdateLayoutProperties(); }} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={t('common:name')} />
              <input type="text" value={props.editNameRu} onChange={(e) => props.setEditNameRu(e.target.value)} className="w-full min-w-0 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent/40 outline-hidden" placeholder={`${t('common:name')} (RU)`} />
              <div>
                <label className="text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1 block">{t('select_block_category')}</label>
                <DropdownSelector
              ...
              <div>
                ...
                <DropdownSelector
                  value={props.editParentTemplateId || props.editingLayout.template.id}
                  onChange={(val) => props.setEditParentTemplateId(val)}
                  options={props.allTemplates.filter(t => t.id !== 'blank-dark').map(tmpl => ({
                    value: tmpl.id,
                    label: `${props.isRu ? tmpl.nameRu : tmpl.name}${tmpl.id === props.editingLayout!.template.id ? ' ●' : ''}`
                  }))}
                  className="py-2.5"
                />
              </div>
