            ...
            {!hideLayerStack && (
                <div className="p-4 border-b border-white/5 bg-black/40">
                    ...
                    <div className="space-y-1 max-h-[180px] overflow-y-auto px-0.5 custom-scrollbar">
                        {layers.map((layer: IStyleLayer, idx: number) => (
                            <div
                                key={layer.id}
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="bg-white/2 rounded-3xl p-6 border border-white/5 space-y-5 shadow-inner">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <CustomSlider label={t('vibrance', 'Vibrance')} min={-100} max={100} step={1} value={activeLayer.adjustments?.vibrance ?? 0} onChange={(v: number) => updateLayer(activeLayer.id, { adjustments: { ...activeLayer.adjustments!, vibrance: v } })} />
                                    <CustomSlider label={t('hue', 'Hue')} min={-180} max={180} step={1} value={activeLayer.adjustments?.hue ?? 0} onChange={(v: number) => updateLayer(activeLayer.id, { adjustments: { ...activeLayer.adjustments!, hue: v } })} unit="°" />
                                    <CustomSlider label={t('blur')} min={0} max={100} step={1} value={activeLayer.adjustments?.blur ?? 0} onChange={(v: number) => updateLayer(activeLayer.id, { adjustments: { ...activeLayer.adjustments!, blur: v } })} unit="px" />
                                </div>
                            </div>
                        </div>
                                        ...
                                        <>
                                            <div className="space-y-2">
                                                <CustomSlider
                                                    ...
                                                    onChange={(v: number) => updateLayer(activeLayer.id, { media: { ...activeLayer.media, speed: v } })}
